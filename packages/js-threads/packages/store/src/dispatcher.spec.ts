...
import { Key, MemoryDatastore, Result } from "interface-datastore"
...
describe("Dispatcher", () => {
  ...
  it("should add new (unique) reducers on registration", async () => {
    ...
    const reducer = {
      ...
      reduce: (...event: Result<TestEvent>[]) => Promise.resolve(undefined),
    }
    d.register(reducer)
  ...
  it("should only dispatch one (set of) events at a time", async () => {
    const d = new Dispatcher()
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const slowReducer = (...event: Result<TestEvent>[]) =>
