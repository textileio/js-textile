...
describe("ed25519", function () {
  ...
  describe("go interop", () => {
    ...
    before(async () => {
      privateKey = await crypto.keys.unmarshalPrivateKey(
        new Uint8Array(fixtures.verify.privateKey)
      )
    })
    ...
    it("verifies with data from go", async () => {
      const key = crypto.keys.unmarshalPublicKey(
        new Uint8Array(fixtures.verify.publicKey)
      )
